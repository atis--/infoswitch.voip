...
var Channel = exports.Channel = (function () {
    ...
    Channel.prototype.onsafe = function (ev, handler) {
        ...
        // emit error
        this.pbx.emit('error', new Error(fmt('Event `%s` bind attempt on terminated channel `%s`',
                                         ev, this.chan)));
    }
    ...
    Channel.prototype.isLive = function (op) {
        if (this.terminated) {
            this.pbx.emit('error', new Error(fmt('`%s` on terminated channel ignored', op)));
            return false;
        }
        return true;
    }
    ...
    Channel.prototype.routeToDestination = function (dest) {
        ...
        this.pbx.routeCall(this.call_route, true, util._extend(fork_params, {
        ...
        this.pbx.onChannelEvent(this.chan, 'onExecuteFork', function (call_execute) {
            ...
            call.pbx.onChannelEvent(sip_id, 'onHangup', function (chan_hangup) {
                ...
                // copy fork channel (outgoing route) disconnect cause to our channel
