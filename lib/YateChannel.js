...
var Channel = exports.Channel = (function () {
    ...
    //
    // get caller/called number
    //
    Channel.prototype.getCaller = function () {
        return this.call_route ? this.call_route.caller : undefined;
    }
    Channel.prototype.getCalled = function () {
        return this.call_route ? this.call_route.called : undefined;
    }
    ...
    Channel.prototype.routeToDestination = function (dest) {
        ...
        // Yate requires SIP headers to be prefixed with "osip_" in the
        // call.route message
        const headers = dest.headers;
        const osip_headers = {};
        if (headers) {
            for (let name in headers)
                osip_headers[`osip_${name}`] = headers[name];
        }
        ...
        this.pbx.routeCall(
            this.call_route,
            true,
            Object.assign(fork_params, osip_headers, {
        ...
        this.pbx.onChannelEvent(this.chan, 'onExecuteFork', function (call_execute) {
            ...
            call.pbx.onChannelEvent(sip_id, 'onConnected', function (chan_connected) {
                ...
                // generate silence tone in case of empty sound queue as soon as
                // peer connects. This is necessary because if the IVR generates
                // no sound, then call recording fails for the peer.
                if (this instanceof IVR && this.queue.length == 0)
    ...
    Channel.prototype.routeToIVR = function (ivropt) {
        ...
        this.pbx.onChannelEvent(this.chan, 'onConnectedOnce', function (chan_connected) {
            ...
            // if nothing in queue, play silence tone to support recording
            if (ivr.queue.length == 0)
...
var IVR = exports.IVR = (function () {
    ...
    function IVR(cfg) {
        ...
        cfg.pbx.onChannelEvent(cfg.chan, 'onNotify', function () {
            ...
            const last_sound = this.queue.shift();

            // check if there are sounds to play
            if (this.queue.length == 0) {
                // play silence if queue becomes empty (unless last sound was
                // already silence)
                if (!last_sound || last_sound.tone != 'silence')
                    this.pbx.chanTonegen(this.chan, 'silence');
