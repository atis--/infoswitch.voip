...
function fork(routes, called) {
    ...
    for (var index = 1; index <= routes.length; index++) {
        ...
        assert(r.host && r.caller, 'missing required parameters from route');

        // let route override `called` number
        if (r.called)
            called = r.called;

        // prepend `sip:` to called number for SIP protocol
        var proto = r.protocol ? r.protocol.toLowerCase() : 'sip';
        var calledfull = (proto == 'sip' ? 'sip:' : '') + called;
...
var Channel = exports.Channel = (function () {
    ...
    Channel.prototype.routeToDestination = function (dest) {
        ...
        this.pbx.reply(this.call_route, true, util._extend(fork(dest.routes, dest.called), {
    ...
    Channel.prototype.routeToIVR = function (ivropt) {
        ...
        this.pbx.onChannelEvent(this.chan, 'onConnectedOnce', function (chan_connected) {
            ...
            var ivr = new IVR({
                chan: dumb_id,
                pbx: this.pbx,
                language: ivropt.language,
                currency: ivropt.currency
            });
