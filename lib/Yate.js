...
// Allow/deny authentication & authorization functions. Note the extra reply
// params that disable auth message handling by "register" and "regfile" Yate
// modules.
const auth_allow = function (user_auth) {
    this.reply(user_auth, true, {
        auth_register: false,
        auth_regfile: false
    });
}
const auth_deny = function (user_auth, err) {
...
module.exports = (function () {
    ...
    //
    // Set "authenticator" function to accept local users and authorize calls.
    //
        ...
        if (this._init_complete) {
            for (const account in old_carriers) {
                if (!new_carriers[account]) {
                    ...
                    // Yate will not notify about a carrier going offline if it
                    // was never online in the first place. For app convenience
                    // we preemptively emit the "carrier-offline" event so that
                    // no extra state tracking is necessary for users.
                    this.emit('carrier-offline', {
                        account,
                        trunk: old_carriers[account],
                        reason: 'unregister'
                    });
                }
            }
    ...
    //     headers:      (optional) object with SIP headers
    // }
    //
    ...
    YateExt.prototype._install_message_handlers = function () {
        ...
        this.on('user.notify', function (user_notify) {
            ...
            if (registered === 'true')
                this.emit('carrier-online', { account, trunk });
            else
                this.emit('carrier-offline', { account, trunk, reason });
        });
        ...
        this.on('user.auth', function (user_auth) {
            const allow = auth_allow.bind(this, user_auth);
            const deny = auth_deny.bind(this, user_auth);
            ...
            const digest = extend_defined({}, {
                newcall: user_auth.newcall == 'true', // true for call authorization
                username: user_auth.username || user_auth.number || user_auth.caller,
            ...
            try {
                this.authenticator(digest, function (err, result) {
                    // process result only if timeout has not occured yet
                    if (timer) {
                        clearTimeout(timer);
                        timer = null;
        ...
        this.on('user.register', function (msg) {
            // User address (route) is the most important piece of info here.
            // Remove ';rinstance' or other params from it.
            const fullroute = (msg.data || '').split(';')[0];
        ...
        this.on('user.unregister', function (msg) {
            this.emit('user-unregister', {
                username: msg.username,
                host: msg.ip_host,
                port: msg.ip_port,
                realm: msg.realm,
        ...
        this.on('call.route', function (call_route) {
            ...
            // extract all non-special headers that Yate provides (the ones with
            // "sip_" prefix)
            const headers = {};
            for (const h in call_route) {
                if (h.startsWith('sip_'))
                    headers[h.substr(4)] = call_route[h];
            }
