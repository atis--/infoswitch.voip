...
module.exports = (function () {
    ...
    //
    YateExt.prototype.setCarriers = function (carriers) {
        assert(carriers instanceof Array);

        //
        // register with supplied carriers
        //
        const new_carriers = [];
        const live_accounts = new Set(this.carriers.map(c => c.account))
        for (let i = 0; i < carriers.length; i++) {
            // carrier account is uniquely identified by "username@host:port"
            const { host, port,
                    username, password,
                    auth_name, auth_domain } = carriers[i];
            const addr = `${host}${port ? ':'+port : ''}`
            const account = `${username ? username+'@' : ''}${addr}`;

            // save the new carrier
            new_carriers.push(Object.assign({}, carriers[i], { account }));

            // submit "user.login" request to Yate for carriers that we do not
            // know about already
            if (this._init_complete && !live_accounts.has(account)) {
                this.dispatch({
                    $name: 'user.login',
                    account,
                    username,
                    ...
                    password,
                    number: username,
                    authname: auth_name || username, // XXX
                    domain: auth_domain || host,     // XXX
                    registrar: addr,
                    outbound: addr,
                });
            }
        ...
        if (this._init_complete) {
            const keep_accounts = new Set(new_carriers.map(c => c.account));
            const old_carriers = this.carriers;
            for (let i = 0; i < old_carriers.length; i++) {
                const { account } = old_carriers[i];
                if (!keep_accounts.has(account)) {
                    this.dispatch({
                        $name: 'user.login',
                        account,
                        protocol: 'sip',
        ...
        var connect_fn = function () {
            ...
            socket.once('close', function () {
                ...
                if (self.reconnectInterval > 0)
                    self._reconnect_timer = setTimeout(connect_fn, self.reconnectInterval);
            });
    ...
    YateExt.prototype._install_message_handlers = function () {
        ...
        this.on('user.auth', function (user_auth) {
            ...
            var digest = extend_defined({}, {
                ...
                address: Misc.extractIP(user_auth)
            });
        ...
        this.on('call.route', function (call_route) {
            ...
            var handled = this.emit('incoming-call', channel, {
                ...
                callerHost: Misc.extractIP(call_route)
            });
